...
\def \countUSA{美国}
\def \villPitt{匹兹堡}
\def \telepata #1#2{在 #1 年#2举行的一系列实验中, 研究人员首先向志愿者展示一些英文单词, 同时记录下其脑部不同区域的活动情况}
\def \location #1{区域 #1}
...
\def \high{高}
\def \low{低}
\def \telepatc #1{之后, 研究人员从另准备的 #1 词列表中选取部分单词, 令志愿者思考, 并再次测量其大脑活动情况}
\def \telepats #1{研究人员称, 前三个因素具有较高的生态效度 #1 和生存价值}
\def \telepati{即实验结果符合真实生活中人类行为的数据}
\def \telepatt #1{而区域 #1 负责印出词语的低级视觉表示}
\def \telepate{下列是志愿者思考的词语对应的四个脑部区域的活跃程度}
\def \telepatd{借助获得的数据, 研究人员能成功判断志愿者思考的词语}
\def \sameinfo{以下是另外六个词语及其对应的志愿者脑部区域活跃程度}

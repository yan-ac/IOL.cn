...
\def \retrflex #1#2#3{#1,~#2 和~#3 $\approx$ 通用北美口音英语 \word{barn}, \word{marsh} 和 \word{art} 中的 \word{b}, \word{sh} and \word{t}, 发音时舌尖向后卷}
\def \syllabir #1{#1~类似普通话和北方方言 word{这儿} (\word{zhèr})中的\word{er}}
\def \burmethe #1#2{#1~$\approx$ 英语\word{with}中的~#2}
\def \glotstop #1{#1~是辅音 (称为声门塞音)}

...
%For IPA
\newfontfamily\TextIPA{CMUSerif-Roman}

\newcommand \ipaword [1]{[\ipa{#1}]}
\newcommand \ipa [1]{\bgroup\TextIPA #1\egroup}
\newcommand \bipa [1]{\bgroup\bfseries\TextIPA #1\egroup}
...
{\small \begin{tabular}[t]{@{}l} \headone\thisYear \itshape 。\\ \slshape \chapname \end{tabular}}\hfill \thepage
\else \thepage \hfill
...
\else
  ...
  \renewcommand \bipa [1]{\bgroup\bfseries\TextIPA #1\egroup}
  \newcommand \zagrad [1]{)#1(}
...
\def \probnumb #1{\ifcase #1\or 一\or 二\or 三\or 四\or 五\fi}

%I don't know why but it seemed that I can't put anything after #1 in \probnumb
\newcommand \problem [1]{\stepcounter {section}\paragraph{\probword \probnumb{\thesection}题\ （20 \pontword）\postpar#1}}
\newcommand \solution [1]{\stepcounter {section}\paragraph{\probword \probnumb{\thesection}题\postpar#1}}
%\def \solution {\stepcounter {section}\paragraph{\probword \probnumb{\thesection}题 \postpar }}
...
\problem {\givesent {\indiCent {\Dyirbal}}\andtrans {\tothislang}：} \mbox{} \medskip\\
%
\begin{tabular}{ll}
...
\problem {\givesent {\inBasque}\andtrans {\tothislang} \chaotict。 \pasoreus：} \mbox{}
%
\begin{center}
\bord{ahaztu ditut,
ahaztu zaizkit,
...
\problem {\giwordss {\ofRotuma}\andtrans {\tothislang}：} \mbox{} \medskip\\
%
...
\synt{A $\to$ V} （\squoted{\adjeverb {\synt A}}）： \L{\synt A\bipa{-man}}。
...
\ergatend
\begin{itemize}
\item \bipa{-ŋgu}， \simotend {\byavowel}\ett {\contains {\twosylla}}；
\item \bipa{-gu}， \simotend {\byavowel}\ett {\contains {\trosylla}}；
...
\item
\begin{enumerate}\setcounter{enumii}{\value{exx}}
\item （\Why\offended {\thecerer}？）\ \teopdaty。\\－ \paaS{A oraoraa}{dao ori bona moon}
\item （\Why\ \teopdaqz？）\ \Upcase \theboy\paaqasun {\bonaiana}{\toraaram}。\\
...
\wordsord
\begin{itemize}
\item \fbox{\synt{N$_1$ N$_2$}} （\synt{N$_1$}： \attribum， \synt{N$_2$}： \attribut）；
\item \fbox{\synt{N A}} （\alsomean {\squoted {\hewhohas {\synt A}{\synt N}}}：
\bord{huag ‘el‘ele} \squoted{\huga\ + \qelqele\ = \huagqele}）；
\item \fbox{\synt{V O}} （\nounverb：
\bord{a‘öf fau} \squoted{\aqofi\ + \fau\ = \aqoffau}，
\bord{hül hạfu} \squoted{\huli\ + \hafu\ = \hulhafu\ （\hurrican）}）。
\end{itemize}
